...
└────────────────────────┬────────────────────────────────────┘
                         ...
                         ▼
┌─────────────────────────────────────────────────────────────┐
│                 Python后端 (OSWorld Agent)                  │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐    │
│  │ Claude   │  │ MCP工具  │  │ 截图识别  │  │ 任务规划  │    │
│  │ API      │  │ 调用     │  │          │  │          │    │
│  └──────────┘  └──────────┘  └──────────┘  └──────────┘    │
└────────────────────────┬────────────────────────────────────┘
...
┌─────────────────────────────────────────────────────────────┐
│                    Windows 真实自动化                        │
│     浏览器、文件操作、鼠标键盘模拟、Office等                  │
└─────────────────────────────────────────────────────────────┘
分工
| 层次 | 技术 | 负责 |
|------|------|------|
| 前端 | aardio | GUI、语音输入、按钮、模型选择、状态显示 |
| 后端 | Python | AI理解、MCP工具调用、任务规划、执行自动化 |
| 通信 | HTTP | JSON数据交换 |
| 底层 | PowerShell | 真实操作Windows |
快速实现步骤
1. 后端API（Python）
# api_server.py
...
async def execute_task(req: TaskRequest):
    # 调用OSWorld Agent
    result = agent.run(req.instruction, req.model)
    return result
@app.post("/screenshot")
...
var client = web.rest.jsonClient("http://127.0.0.1:8000");
// 执行任务
var result = client.post("/execute", {
    instruction = "打开Chrome浏览器，访问百度",
    model = "claude-sonnet-4-5"
});
// 展示结果
winform.resultText = result.message;
3. 启动脚本
# start.bat
REM 启动Python后端
...
一款多平台媒体数据爬虫技能，支持小红书、抖音、知乎、微博、B站等平台的内容和评论爬取。它提供环境检查、自动化安装、简化命令行调用等功能，支持多种数据存储格式（CSV/JSON/Excel/SQLite/MySQL/MongoDB），适用于数据采集、舆情分析、内容研究等学习场景。
### 原版英文 Gemini 3 系统提示词
