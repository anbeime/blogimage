...
- 在 aardio 代码编辑器中按 `F1` 键可激活 AI 编码助手，它有四种工作模式：
    ...
    * 代码续写模式：如果按 `F1` 时无选区，AI 编码助手将帮您续写或补全代码
    ...
    > 注意 `F1` 键助手会自动获取代码上下文，再去手动复制粘贴上下文代码是多余的（ 这会混淆上下文代码与用户提示，也不必要地增加上下文了长度，并导致回复质量下降 ）
...
## 设置合适的参数 <a id="temperature" href="#temperature">&#x23;</a>
...
对于传统的非思考类大模型处理编程这种严谨并重视精确性的任务，一般建议使用较低的 temperature，例如 0 或者 0.1 。如果设置较高的 temperature 则 AI 很可能会更多地胡编乱造，虚构各种库、函数、甚至是天马行空地生造语法。

但是对于现代思考类大模型，通常会要求将 temperature 设为 1。
最典型的就是 Gemini 3 Flash 或者 Gemini 3 Pro 必须设为 1,设为其他值生成质量会显著下降。
这是因为思考类模型需要获取更多可能性以生成最优结果。

> 一般不建议改动大模型除 temperature 之外其他参数的默认值，例如 top-p。
> 对于 Gemini 3 Flash 等现代思考类模型， temperature 也应保持默认值 1 。

对于思考类模型，合适地设置推理强度则非常重要。如果是在 aardio 中使用 `web.rest.aiChat` 对象，则可以通过构造参数表中的 `reasoning` 字段统一设置，请参考 [设置 reasoning 参数](../../library-guide/std/web/rest/aiChat.html#reasoning) 。
