...
本节罗列了ArkTS不支持或部分支持的TypeScript特性。完整的列表以及详细的代码示例和重构建议，请参考[约束说明](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/typescript-to-arkts-migration-guide#%E7%BA%A6%E6%9D%9F%E8%AF%B4%E6%98%8E)。更多案例请参考[适配指导案例](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-more-cases)。
...
15. let res: CallResult = some_api_function('hello', 'world');
16. if (!res.succeeded()) {
17.   console.info('Call failed: ' + res.errorMessage());
18. }
...
9. // 无法从对象中删除某个属性，从而确保所有Point对象都具有属性x
10. let p1 = new Point(1.0, 1.0);
11. delete p1.x;           // 在TypeScript和ArkTS中，都会产生编译时错误
12. delete (p1 as any).x;  // 在TypeScript中不会报错；在ArkTS中会产生编译时错误

13. // Point类没有定义命名为z的属性，在程序运行时也无法添加该属性
14. let p2 = new Point(2.0, 2.0);
15. p2.z = 'Label';           // 在TypeScript和ArkTS中，都会产生编译时错误
16. (p2 as any).z = 'Label';   // 在TypeScript中不会报错；在ArkTS中会产生编译时错误

17. // 类的定义确保了所有Point对象只有属性x和y，并且无法被添加其他属性
18. let p3 = new Point(3.0, 3.0);
19. let prop = Symbol();      // 在TypeScript中不会报错；在ArkTS中会产生编译时错误
20. (p3 as any)[prop] = p3.x; // 在TypeScript中不会报错；在ArkTS中会产生编译时错误
21. p3[prop] = p3.x;          // 在TypeScript和ArkTS中，都会产生编译时错误

22. // 类的定义确保了所有Point对象的属性x和y都具有number类型，因此，无法将其他类型的值赋值给它们
23. let p4 = new Point(4.0, 4.0);
24. p4.x = 'Hello!';          // 在TypeScript和ArkTS中，都会产生编译时错误
25. (p4 as any).x = 'Hello!'; // 在TypeScript中不会报错；在ArkTS中会产生编译时错误

26. // 使用符合类定义的Point对象：
27. function distance(p1: Point, p2: Point): number {
28.   return Math.sqrt(
29.     (p2.x - p1.x) * (p2.x - p1.x) + (p2.y - p1.y) * (p2.y - p1.y)
30.   );
31. }
32. let p5 = new Point(5.0, 5.0);
33. let p6 = new Point(6.0, 6.0);
34. console.info('Distance between p5 and p6: ' + distance(p5, p6));
...
为获得更好的性能并鼓励开发者编写更清晰的代码，ArkTS限制了一些运算符的语义。详细的语义限制，请参考[约束说明](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/typescript-to-arkts-migration-guide#%E7%BA%A6%E6%9D%9F%E8%AF%B4%E6%98%8E)。
...
3.   public greet(): void {
4.     console.info('Hello, ' + this.name);
5.   }
6. }
...
9.   public greet(): void {
10.     console.info('Greetings, ' + this.name);
11.   }
12. }
...
鉴于此，当前我们还不支持该特性。根据实际场景的需求和反馈，我们后续会重新加以考虑。更多案例和建议请参考[约束说明](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/typescript-to-arkts-migration-guide#%E7%BA%A6%E6%9D%9F%E8%AF%B4%E6%98%8E)。
...
2. console.info(x['name']); // x
3. console.info(x[2]); // 3
...
6. let y = ['a', 'b', 'c'];
7. console.info(y[2]); // c

8. // 在需要通过非标识符（即不同类型的key）获取数据的场景中，使用Map<Object, some_type>。
9. let z = new Map<Object, string>();
10. z.set('name', '1');
11. z.set(2, '2');
12. console.info(z.get('name'));  // 1
13. console.info(z.get(2)); // 2
...
18. let obj: Record<string, number> = {
19.   [Test.A]: 1,   // 枚举中的字符串值
20.   [Test.B]: 2,   // 枚举中的字符串值
21.   ['value']: 3   // 字符串字面量
22. }
...
7. console.info(f(true));  // b
8. console.info(f(false)); // undefined
...
4. let value2: unknown
5. value2 = true;
6. value2 = 42;

**ArkTS**

1. let value_b: boolean = true; // 或者 let value_b = true
2. let value_n: number = 42; // 或者 let value_n = 42
3. let value_o1: Object = true;
4. let value_o2: Object = 42;
...
5. function doSomething(fn: DescribableFunction): void {
6.   console.info(fn.description + ' returned ' + fn(''));
7. }
...
10. function doSomething(fn: DescribableFunction): void {
11.   console.info(fn.description + ' returned ' + fn.invoke(''));
12. }

13. doSomething(new DescribableFunction());
...
2. type SomeConstructor = {
3.   new (s: string): SomeObject
4. }

5. function fn(ctor: SomeConstructor) {
6.   return new ctor('hello');
7. }
...
7. function fn(s: string): SomeObject {
8.   return new SomeObject(s);
9. }
...
5. function getStringArray(): StringArray {
6.   return ['a', 'b', 'c'];
7. }

8. const myArray: StringArray = getStringArray();
9. const secondItem = myArray[1];
...
4. let myArray: X = new X();
5. const secondItem = myArray.f[1];
...
9. type Employee = Identity & Contact
...
9. interface Employee extends Identity,  Contact {}
...
3. // 用Object重写，类型控制较少，需要更多的类型检查以确保安全
4. type X2<T> = Object

5. // Item必须作为泛型参数使用，并能正确实例化
6. type YI<Item, T extends Array<Item>> = Item
...
12.   constructor(name: string, age: number, email: string,
...
20. let person = new Person('John', 30, '***@example.com', '18*********');
21. console.info(person['name']);     // 编译时错误
22. console.info(person.unknownProperty); // 编译时错误

23. let arr = new Int32Array(1);
24. arr[0];
...
21. function foo(x: X) {
22.   console.info(x.n + x.s);
23. }

24. // 由于X和Y的API是等价的，所以X和Y是等价的
25. foo(new X());
26. foo(new Y());
...
17. console.info('Assign D to B');
18. b = d; // 合法赋值，因为B是D的父类
...
25. // 类X implements 接口Z，构建了X和Y的关系
26. class X implements Z {
27.   n: number = 0
28.   s: string = ''
29. }

30. // 类Y implements 接口Z，构建了X和Y的关系
31. class Y implements Z {
32.   n: number = 0
33.   s: string = ''
34. }
...
37. console.info('Assign X to Y');
38. y = x // 合法赋值，它们是相同的类型

39. console.info('Assign Y to X');
40. x = y // 合法赋值，它们是相同的类型

41. function foo(c: Z): void {
42.   console.info(c.n + c.s);
43. }

44. // 类X和类Y implement 相同的接口，因此下面的两个函数调用都是合法的
45. foo(new X());
46. foo(new Y());
...
4. let x = choose(10, 20);   // 推断choose<number>(...)
5. let y = choose('10', 20); // 编译时错误

6. function greet<T>(): T {
7.   return 'Hello' as T;
8. }
9. let z = greet() // T的类型被推断为“unknown”
...
4. let oo: Object[] = [{n: 1, s: '1'}, {n: 2, s: '2'}];
...
5. let o1: C1 = {n: 42, s: 'foo'};
6. let o2: C1 = {n: 42, s: 'foo'};
7. let o3: C1 = {n: 42, s: 'foo'};

8. let oo: C1[] = [{n: 1, s: '1'}, {n: 2, s: '2'}];
...
5. function getPoint(o: Point): Point {
6.   return o;
7. }

8. // TS支持structural typing，可以推断p的类型为Point
9. let p = {x: 5, y: 10};
10. getPoint(p);

11. // 可通过上下文推断出对象字面量的类型为Point
12. getPoint({x: 5, y: 10});
...
4.   // 在字面量初始化之前，使用constructor()创建一个有效对象。
5.   // 由于没有为Point定义构造函数，编译器将自动添加一个默认构造函数。
6. }

7. function getPoint(o: Point): Point {
8.   return o;
9. }
...
13. // getPoint接受Point类型，字面量初始化生成一个Point的新实例
14. getPoint({x: 5, y: 10});
...
5. type S = Set<{x: number, y: number}>
...
5. let a1 = [{n: 1, s: '1'} as C, {n: 2, s: '2'} as C]; // a1的类型为“C[]”
6. let a2: C[] = [{n: 1, s: '1'}, {n: 2, s: '2'}];    // a2的类型为“C[]”
...
9. const rectangle = new Rectangle(0.0, 0.0);
...
9. const rectangle = new testRectangle(0.0, 0.0);
...
6. class Derived extends C {
7.   foo() {
8.     console.info('Extra');
9.     super.foo();
10.   }
11. }
...
4. function createShape(): testShape {
5.   return new testCircle();
6. }

7. let c1 = <testCircle> createShape();

8. let c2 = createShape() as testCircle;

9. // 如果转换错误，不会产生编译时或运行时报错
10. let c3 = createShape() as testSquare;
11. console.info(c3.y); // undefined

12. // 在TS中，由于`as`关键字不会在运行时生效，所以`instanceof`的左操作数不会在运行时被装箱成引用类型
13. let e1 = (5.0 as Number) instanceof Number; // false

14. // 创建Number对象，获得预期结果：
15. let e2 = (new Number(5.0)) instanceof Number; // true
...
3. function createShape(): testShape {
4.   return new testCircle();
5. }

6. let c1 = createShape() as testCircle;

7. // 创建Number对象，获得预期结果：
8. let e1 = (new Number(5.0)) instanceof Number; // true
...
8. function returnTen(): string {
9.   return '-10';
10. }

11. function returnString(): string {
12.   return 'string';
13. }

14. let x = +returnTen();  // -10（number类型）
15. let y = +returnString(); // NaN
...
14. let x = +returnTen();  // 编译时错误
15. let y = +returnString(); // 编译时错误
...
5. let b = p instanceof Person; // true，且属性name一定存在
...
5. function returnZeroPoint(): Point {
6.   return new Point();
7. }

8. let {x, y} = returnZeroPoint();
...
8. // 只有在开启noImplicitAny选项时会产生编译时错误
9. function g(x: number) {
10.   return f(x - 1);
11. }

12. function doOperation(x: number, y: number) {
13.   return x + y;
14. }
...
8. // 可以省略返回类型，返回类型可以从f的类型标注推导得到
9. function g(x: number): number {
10.   return f(x - 1);
11. }

12. // 可以省略返回类型
13. function doOperation(x: number, y: number) {
14.   return x + y;
15. }
...
7. drawText({ text: 'Hello, world!', location: [100, 50], bold: true });
...
9. function main() {
10.   drawText('Hello, world!', [100, 50], true);
11. }
...
2.   // 函数内声明函数
3.   function logToConsole(message: string): void {
...
7.   // 调用函数
8.   logToConsole('result is ' + result);
9. }
...
7.   logToConsole('result is ' + result);
8. }
...
8. let a = new A();
9. console.info(a.count); // 打印a
10. a.m('b');
11. console.info(a.count); // 打印b
...
6. for (let num of counter(1, 5)) {
7.   console.info(num.toString());
8. }
...
6.   for (let i = 1; i <= 5; i++) {
7.     await complexNumberProcessing(i);
8.   }
9. }
...
9. function isFoo(arg: any): arg is Foo {
10.   return arg.foo !== undefined;
11. }

12. function doStuff(arg: Foo | Bar) {
13.   if (isFoo(arg)) {
14.     console.info(arg.foo);  // OK
15.     console.info(arg.bar);  // 编译时错误
16.   } else {
17.     console.info(arg.foo);  // 编译时错误
18.     console.info(arg.bar);  // OK
19.   }
20. }

21. doStuff({ foo: '123', common: '123' });
22. doStuff({ bar: '123', common: '123' });
...
9. function isFoo(arg: Object): boolean {
10.   return arg instanceof Foo;
11. }

12. function doStuff(arg: Object): void {
13.   if (isFoo(arg)) {
14.     let fooArg = arg as Foo;
15.     console.info(fooArg.foo);   // OK
16.     console.info(arg.bar);    // 编译时错误
17.   } else {
18.     let barArg = arg as Bar;
19.     console.info(arg.foo);    // 编译时错误
20.     console.info(barArg.bar);   // OK
21.   }
22. }
...
4. let args: [number, number, number] = [0, 1, 2];
5. foo(...args);

**ArkTS**

1. function log_numbers(x: number, y: number, z: number) {
2.   // ...
3. }

4. let numbers: number[] = [1, 2, 3];
5. log_numbers(numbers[0], numbers[1], numbers[2]);
...
1. class Point2D {
2.   x: number = 0; y: number = 0
3. }

4. class Point3D {
5.   x: number = 0; y: number = 0; z: number = 0
6.   constructor(p2d: Point2D, z: number) {
7.     this.x = p2d.x;
8.     this.y = p2d.y;
9.     this.z = z;
10.   }
...
12. let p3d = new Point3D({ x: 1, y: 2 } as Point2D, 3);

13. class DerivedFromArray extends Uint16Array {};

14. let arr1 = [1, 2, 3];
15. let arr2 = new Uint16Array([4, 5, 6]);
16. let arr3 = new DerivedFromArray([7, 8, 9]);
17. let arr4 = [...arr1, 10, ...arr2, 11, ...arr3];
...
7. interface MoverShaker extends Mover, Shaker {
8.   getStatus(): {
9.     speed: number
10.     frequency: number
11.   }
...
13. class C implements MoverShaker {
14.   private speed: number = 0
15.   private frequency: number = 0

16.   getStatus() {
17.     return { speed: this.speed, frequency: this.frequency };
18.   }
19. }
...
10. class ShakeStatus {
11.   public frequency: number
12.   constructor() {
13.     this.frequency = 0;
14.   }
15. }
16. interface Shaker {
17.   getShakeStatus(): ShakeStatus
18. }

19. class MoveAndShakeStatus {
20.   public speed: number
21.   public frequency: number
22.   constructor() {
23.     this.speed = 0;
24.     this.frequency = 0;
25.   }
...
27. class C implements Mover, Shaker {
28.   private move_status: MoveStatus
29.   private shake_status: ShakeStatus

30.   constructor() {
31.     this.move_status = new MoveStatus();
32.     this.shake_status = new ShakeStatus();
33.   }

34.   public getMoveStatus(): MoveStatus {
35.     return this.move_status;
36.   }

37.   public getShakeStatus(): ShakeStatus {
38.     return this.shake_status;
39.   }

40.   public getStatus(): MoveAndShakeStatus {
41.     return {
42.       speed: this.move_status.speed,
43.       frequency: this.shake_status.frequency
44.     };
45.   }
46. }
...
5.   createElement(tagName: string): boolean;
6. }

7. interface Document {
8.   createElement(tagName: number): number;
9.   createElement(tagName: boolean): boolean;
10.   createElement(tagName: string, value: number): string;
11. }
...
8. function createPerson(Ctor: PersonCtor, name: string, age: number): Person
9. {
10.   return new Ctor(name, age);
11. }

12. const person = createPerson(Person, 'John', 30);
...
8. function createPerson(Ctor: PersonCtor, n: string, a: number): Person {
9.   return Ctor(n, a);
10. }

11. let Impersonizer: PersonCtor = (n: string, a: number): Person => {
12.   return new Person(n, a);
13. }

14. const person = createPerson(Impersonizer, 'John', 30);
...
3. class Point {
4.   constructor(x: number, y: number) {}
5.   static origin = new Point(0, 0)
6. }

7. // module2
8. import Pt = require('module1')

9. let p = Pt.Point.origin;
...
6. // module2
7. import * as Pt from 'module1'

8. let p = Pt.Point.origin
...
6. // 使用代码
7. import fileContent from 'some.txt!text'
...
5. // 使用代码
6. import * as m from 'module'
7. console.info('N.foo called: ' + N.foo(42));
...
5. // 程序中
6. import { mathLib } from 'math-lib'
7. mathLib.isPrime(2)
...
4. C.prototype = {
5.   m() {
6.     console.info(this.p);
7.   }
8. }

9. C.prototype.q = function(r: string) {
10.   return this.p == r;
11. }
...
10. let n: number = null; // 只有在开启strictNullChecks选项时会产生编译时错误
...
5. let s1: string = null; // 没有报错

6. // @ts-ignore
7. let s2: string = null; // 没有报错
...
6. import('module2').then(() => {}).catch(() => {})  // 动态import
...
4. // main.ets
5. let e0: ESObject = foo(); // API18以前，编译时错误：ESObject类型只能用于局部变量；API18以后，OK，显式标注ESObject类型

6. function f() {
7.   let e1 = foo();        // 编译时错误：e1的类型是any
8.   let e2: ESObject = 1;  // API18以前，编译时错误：不能用非动态值初始化ESObject类型变量；API18以后，OK，支持赋值数字类型
9.   let e3: ESObject = {}; // API18以前，编译时错误：不能用非动态值初始化ESObject类型变量；API18以后，编译时错误：ESObject不支持赋值对象字面量类型
10.   let e4: ESObject = []; // API18以前，编译时错误：不能用非动态值初始化ESObject类型变量；API18以后，OK，支持赋值数组类型
11.   let e5: ESObject = ''; // API18以前，编译时错误：不能用非动态值初始化ESObject类型变量；API18以后，OK，支持赋值字符串类型
12.   e5['prop'];            // API18以前，编译时错误：不能访问ESObject类型变量的属性；API18以后，OK，支持[]访问
13.   e5[1];                 // API18以前，编译时错误：不能访问ESObject类型变量的属性；API18以后，OK，支持[]访问
14.   e5.prop;               // API18以前，编译时错误：不能访问ESObject类型变量的属性；API18以后，OK，支持点操作符访问

15.   let e6: ESObject = foo(); // OK，显式标注ESObject类型
16.   let e7: ESObject = e6;    // OK，使用ESObject类型赋值
17.   bar(e7);                  // OK，ESObject类型变量传给跨语言调用的函数
18. }
# ArkWeb组件安全开发

更新时间: 2025-12-12 19:30
...
8.   urlTrustList: string =
9.     '{\'UrlPermissionList\':[{\'scheme\':\'https\', \'host\':\'trust.example.com\', \'port\':80, \'path\':\'test\'}]}'

10.   build() {
11.     Column() {
12.       Button('Setting the trustList')
13.         .onClick(() => {
14.           try {
15.             // Set up an allowlist to allow access only to trusted web pages.
16.             this.controller.setUrlTrustList(this.urlTrustList);
17.           } catch (error) {
18.             hilog.error(0x0000, 'ArkWebSecurity',
19.               `ErrorCode: ${(error as BusinessError).code}, Message: ${(error as BusinessError).message}`);
20.           }
21.         })
22.       Button('Access the trust web')
23.         .onClick(() => {
24.           try {
25.             // Allowlist activated, access to trusted web pages is permitted.
26.             this.controller.loadUrl('https://trust.example.com/test');
27.           } catch (error) {
28.             hilog.error(0x0000, 'ArkWebSecurity',
29.               `ErrorCode: ${(error as BusinessError).code}, Message: ${(error as BusinessError).message}`);
30.           }
31.         })
32.       Button('Access the untrusted web')
33.         .onClick(() => {
34.           try {
35.             // Allowlist activated, blocking access to untrusted web and displaying an alarm page.
36.             this.controller.loadUrl('http://untrust.example.com/test');
37.           } catch (error) {
38.             hilog.error(0x0000, 'ArkWebSecurity',
39.               `ErrorCode: ${(error as BusinessError).code}, Message: ${(error as BusinessError).message}`);
40.           }
41.         })
42.     }
43.   }
...
[SetURLTrustList.ets](https://gitee.com/harmonyos_samples/BestPracticeSnippets/blob/master/ArkWebSecurity/arkwebsecurity/src/main/ets/pages/SetURLTrustList.ets#L17-L62)
...
8.   urlTrustList: string = '{\'UrlPermissionList\':'
9.     + '[{\'scheme\':\'https\', \'host\':\'cdnjs.cloudflare.com\'},' // Public CDN domains are untrusted.
10.     + '{\'scheme\':\'https\', \'host\':\'cdn.ampproject.org\'}]}' // Public service domains are at risk of being abused.
11.   urlStr: string = '' // Any URL to be loaded.

12.   build() {
13.     Column() {
14.       Web({ src: this.urlStr, controller: this.webviewController })
15.         .javaScriptAccess(true)
16.         .onControllerAttached(() => {
17.           try {
18.             this.webviewController.setUrlTrustList(this.urlTrustList);
19.           } catch (error) {
...
[SetURLTrustList.ets](https://gitee.com/harmonyos_samples/BestPracticeSnippets/blob/master/ArkWebSecurity/arkwebsecurity/src/main/ets/pages/ErrorSamples/SetURLTrustList.ets#L17-L44)
...
[LoadURL.ets](https://gitee.com/harmonyos_samples/BestPracticeSnippets/blob/master/ArkWebSecurity/arkwebsecurity/src/main/ets/pages/ErrorSamples/LoadURL.ets#L29-L47)
...
[index.html](https://gitee.com/harmonyos_samples/BestPracticeSnippets/blob/master/ArkWebSecurity/arkwebsecurity/src/main/resources/rawfile/index.html#L18-L29)
...
[LoadURL.ets](https://gitee.com/harmonyos_samples/BestPracticeSnippets/blob/master/ArkWebSecurity/arkwebsecurity/src/main/ets/pages/LoadURL.ets#L29-L53)
...
14.       // Intercept URL resource access through onInterceptRequest and customize the response message.
15.         .onInterceptRequest((event) => {
16.           // The URL passed by event.request.getRequestUrl() should be restricted to a specific path,
17.           // and the presence of '../' should be checked.
18.           if (event.request.getRequestUrl().startsWith('file:///data/trusted_path') &&
19.             event.request.getRequestUrl().includes('../') !== true) {
20.             try {
21.               let file =
22.                 fileIo.openSync(event.request.getRequestUrl(), fileIo.OpenMode.READ_ONLY | fileIo.OpenMode.CREATE);
23.               let arrayBuffer: ArrayBuffer = new ArrayBuffer(6);
24.               fileIo.readSync(file.fd, arrayBuffer, { offset: 0, length: arrayBuffer.byteLength });
25.               fileIo.closeSync(file);
26.               let decoder = util.TextDecoder.create('utf-8');
27.               let stringData = decoder.decodeToString(new Uint8Array(arrayBuffer));
28.               let head1: Header = {
29.                 headerKey: 'Connection',
30.                 headerValue: 'keep-alive'
31.               }
32.               let head2: Header = {
33.                 headerKey: 'Cache-Control',
34.                 headerValue: 'no-cache'
35.               }
36.               this.heads.push(head1);
37.               this.heads.push(head2);

38.               const promise: Promise<String> = new Promise((resolve: Function) => {
39.                 this.responseWeb.setResponseHeader(this.heads);
40.                 // After the file is read, it will be encapsulated and the response will be called back to the web page.
41.                 this.responseWeb.setResponseData(stringData);
...
47.               })
48.               promise.then(() => {
49.                 hilog.info(0x0000, 'ArkWebSecurity', 'prepare response ready');
50.                 this.responseWeb.setResponseIsReady(true);
51.               })
52.               this.responseWeb.setResponseIsReady(false);
53.             } catch (error) {
54.               hilog.error(0x0000, 'ArkWebSecurity', `ErrorCode: ${error.code}, Message: ${error.message}`);
55.             }
56.           }
57.           return this.responseWeb;
58.         })
59.     }
60.   }
61. }

[OnInterceptRequest.ets](https://gitee.com/harmonyos_samples/BestPracticeSnippets/blob/master/ArkWebSecurity/arkwebsecurity/src/main/ets/pages/OnInterceptRequest.ets#L17-L80)
...
[MixedMode.ets](https://gitee.com/harmonyos_samples/BestPracticeSnippets/blob/master/ArkWebSecurity/arkwebsecurity/src/main/ets/pages/ErrorSamples/MixedMode.ets#L26-L27)
...
[MixedMode.ets](https://gitee.com/harmonyos_samples/BestPracticeSnippets/blob/master/ArkWebSecurity/arkwebsecurity/src/main/ets/pages/MixedMode.ets#L26-L27)
...
9.         .onSslErrorEvent((event: SslErrorEvent) => {
10.           console.log('ssl check failed, error is: ' + event.error.toString());
11.           event.handler.handleConfirm();
12.         })
13.     }
14.   }
15. }

[OnSslErrorEventReceive.ets](https://gitee.com/harmonyos_samples/BestPracticeSnippets/blob/master/ArkWebSecurity/arkwebsecurity/src/main/ets/pages/ErrorSamples/OnSslErrorEventReceive.ets#L17-L33)
...
10.         .onSslErrorEvent((event: SslErrorEvent) => {
11.           hilog.info(0x0000, 'ArkWebSecurity', 'onSslErrorEvent url: ' + event.url);
12.           hilog.info(0x0000, 'ArkWebSecurity', 'onSslErrorEvent error: ' + event.error);
13.           hilog.info(0x0000, 'ArkWebSecurity', 'onSslErrorEvent originalUrl: ' + event.originalUrl);
14.           hilog.info(0x0000, 'ArkWebSecurity', 'onSslErrorEvent referrer: ' + event.referrer);
15.           hilog.info(0x0000, 'ArkWebSecurity', 'onSslErrorEvent isFatalError: ' + event.isFatalError);
16.           hilog.info(0x0000, 'ArkWebSecurity', 'onSslErrorEvent isMainFrame: ' + event.isMainFrame);
17.           this.getUIContext().showAlertDialog({
18.             title: 'onSslErrorEvent',
19.             message: 'text',
20.             primaryButton: {
21.               value: 'Confirm',
22.               action: () => {
23.                 event.handler.handleConfirm();
24.               }
25.             },
26.             secondaryButton: {
27.               value: 'cancel',
28.               action: () => {
29.                 event.handler.handleCancel();
30.               }
31.             },
32.             cancel: () => {
33.               event.handler.handleCancel();
34.             }
35.           })
36.         })
37.     }
...
[OnSslErrorEventReceive.ets](https://gitee.com/harmonyos_samples/BestPracticeSnippets/blob/master/ArkWebSecurity/arkwebsecurity/src/main/ets/pages/OnSslErrorEventReceive.ets#L17-L57)
...
[WebDebug.ets](https://gitee.com/harmonyos_samples/BestPracticeSnippets/blob/master/ArkWebSecurity/arkwebsecurity/src/main/ets/pages/ErrorSamples/WebDebug.ets#L26-L30)
...
参考文档：[建立应用侧与H5侧交互通道](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/web-in-page-app-function-invoking#%E5%A6%82%E4%BD%95%E5%BB%BA%E7%AB%8B%E5%BA%94%E7%94%A8%E4%BE%A7%E4%B8%8Eh5%E4%BE%A7%E7%9A%84%E4%BA%A4%E4%BA%92%E9%80%9A%E9%81%93)
...
3. class InsecureObj {
4.   executeSQL(cmd: string) {
5.     // Do something dangerous here
6.     hilog.info(0x0000, 'ArkWebSecurity', 'Execute: ' + cmd);
7.   }
8. }
...
[JavaScriptProxy.ets](https://gitee.com/harmonyos_samples/BestPracticeSnippets/blob/master/ArkWebSecurity/arkwebsecurity/src/main/ets/pages/ErrorSamples/JavaScriptProxy.ets#L17-L45)
...
10.         .onOverrideUrlLoading((webResourceRequest: WebResourceRequest) => {
11.           if (webResourceRequest && webResourceRequest.getRequestUrl() === 'http://www.example.com') {
12.             try {
13.               this.controller.loadUrl('www.example.com');
14.               return true;
15.             } catch (error) {
16.               hilog.error(0x0000, 'ArkWebSecurity', `ErrorCode: ${error.code}, Message: ${error.message}`);
17.             }
18.           }
19.           return false;
20.         })
...
[OnOverrideUrlLoading.ets](https://gitee.com/harmonyos_samples/BestPracticeSnippets/blob/master/ArkWebSecurity/arkwebsecurity/src/main/ets/pages/ErrorSamples/OnOverrideUrlLoading.ets#L17-L41)
...
[GetURL.ets](https://gitee.com/harmonyos_samples/BestPracticeSnippets/blob/master/ArkWebSecurity/arkwebsecurity/src/main/ets/pages/ErrorSamples/GetURL.ets#L29-L35)
...
[GetURL.ets](https://gitee.com/harmonyos_samples/BestPracticeSnippets/blob/master/ArkWebSecurity/arkwebsecurity/src/main/ets/pages/GetURL.ets#L29-L35)
...
3. class TestObj {
4.   controller: webview.WebviewController

5.   constructor(webview_controller: webview.WebviewController) {
6.     this.controller = webview_controller
7.   }

8.   goto(uri: string) {
9.     try {
10.       this.controller.loadUrl(uri);
11.     } catch (error) {
12.       hilog.error(0x0000, 'ArkWebSecurity', `ErrorCode: ${error.code}, Message: ${error.message}`);
13.     }
14.   }
15. }
...
20.   testObj = new TestObj(this.controller);
...
[Redirection.ets](https://gitee.com/harmonyos_samples/BestPracticeSnippets/blob/master/ArkWebSecurity/arkwebsecurity/src/main/ets/pages/ErrorSamples/Redirection.ets#L17-L54)
...
6.   constructor(webview_controller: webview.WebviewController) {
7.     this.controller = webview_controller
8.   }

9.   eval(uri: string) {
10.     this.controller.runJavaScript(uri, (error) => {
11.       if (error) {
12.         hilog.error(0x0000, 'ArkWebSecurity',
13.           `run JavaScript error, ErrorCode: ${(error as BusinessError).code},  Message: ${(error as BusinessError).message}`);
14.         return;
15.       }
16.     });
17.     return 'AceString';
...
24.   testObj: TestObj = new TestObj(this.controller);

25.   build() {
26.     Column() {
27.       Web({ src: 'www.example.com', controller: this.controller })
28.         .javaScriptAccess(true)
29.         .javaScriptProxy({
30.           object: this.testObj,
31.           name: 'objName',
32.           methodList: ['eval'],
...
[RunJavaScript.ets](https://gitee.com/harmonyos_samples/BestPracticeSnippets/blob/master/ArkWebSecurity/arkwebsecurity/src/main/ets/pages/ErrorSamples/RunJavaScript.ets#L17-L59)
...
10.     let atManager = abilityAccessCtrl.createAtManager();
11.     let context: Context = this.getUIContext().getHostContext() as common.UIAbilityContext;
12.     atManager.requestPermissionsFromUser(context, ['ohos.permission.CAMERA', 'ohos.permission.MICROPHONE'])
13.       .then((data) => {
14.         hilog.info(0x0000, 'ArkWebSecurity', 'data: ' + JSON.stringify(data));
15.         hilog.info(0x0000, 'ArkWebSecurity', 'data permission: ' + data.permissions);
16.         hilog.info(0x0000, 'ArkWebSecurity', 'data authResults: ' + data.authResults);
17.       }).catch((error: BusinessError) => {
18.       hilog.error(0x0000, 'ArkWebSecurity',
19.         `Failed to request permissions from user. Code is ${error.code}, message is ${error.message}`);
20.     })
21.   }

22.   build() {
23.     Column() {
24.       Web({ src: 'https://example.com/index.html', controller: this.controller })
25.         .onPermissionRequest((event) => {
26.           // Directly granting page permissions is a wrong approach.
27.           event.request.grant(event.request.getAccessibleResource());
28.         })
29.     }
30.   }
31. }

[PermissionRequest.ets](https://gitee.com/harmonyos_samples/BestPracticeSnippets/blob/master/ArkWebSecurity/arkwebsecurity/src/main/ets/pages/ErrorSamples/PermissionRequest.ets#L17-L50)
...
[index.html](https://gitee.com/harmonyos_samples/BestPracticeSnippets/blob/master/ArkWebSecurity/arkwebsecurity/src/main/resources/rawfile/index.html#L33-L60)
...
25.         .onPermissionRequest((event) => {
26.           if (event) {
27.             this.getUIContext().showAlertDialog({
28.               title: 'title',
29.               message: 'text',
30.               primaryButton: {
31.                 value: 'deny',
32.                 action: () => {
33.                   event.request.deny();
34.                 }
35.               },
36.               secondaryButton: {
37.                 value: 'onConfirm',
38.                 action: () => {
39.                   // Explicit pop-ups that ask user for authorization, e.g., using AlertDialog, is the correct approach.
...
[PermissionRequest.ets](https://gitee.com/harmonyos_samples/BestPracticeSnippets/blob/master/ArkWebSecurity/arkwebsecurity/src/main/ets/pages/PermissionRequest.ets#L17-L70)
...
9.         .geolocationAccess(true)
10.         .onGeolocationShow((event) => {
11.           event.geolocation.invoke(event.origin, true, true);
12.         })
13.     }
14.   }
15. }

[GeolocationShow.ets](https://gitee.com/harmonyos_samples/BestPracticeSnippets/blob/master/ArkWebSecurity/arkwebsecurity/src/main/ets/pages/ErrorSamples/GeolocationShow.ets#L17-L33)
...
16.                 value: 'onConfirm',
17.                 action: () => {
18.                   event.geolocation.invoke(event.origin, true, true);
19.                 }
20.               },
21.               cancel: () => {
22.                 event.geolocation.invoke(event.origin, false, true);
23.               }
24.             })
25.           }
26.         })
...
[GeolocationShow.ets](https://gitee.com/harmonyos_samples/BestPracticeSnippets/blob/master/ArkWebSecurity/arkwebsecurity/src/main/ets/pages/GeolocationShow.ets#L17-L47)
...
[RunJavaScript2.ets](https://gitee.com/harmonyos_samples/BestPracticeSnippets/blob/master/ArkWebSecurity/arkwebsecurity/src/main/ets/pages/ErrorSamples/RunJavaScript2.ets#L35-L49)
...
6. struct WebComponent {
7.   controller: webview.WebviewController = new webview.WebviewController();
8.   dataFromH5: string = '' // Messages passed from web pages
...
15.         .onPageEnd(event => {
16.           try {
17.             // Use regular expressions to match all single and double quotes.
18.             const regex = /['"\(\)]/g;
19.             // Use the replace method to replace all matched characters with an empty string.
20.             let sanitizedStr = this.dataFromH5.replace(regex, '');
21.             this.controller.runJavaScript(
22.               // Perform security filtering on the data_from_H5 passed through the web page before calling back.
23.               'javascript:alert(' + sanitizedStr + ')',
24.               (error, result) => {
25.                 if (error) {
26.                   hilog.error(0x0000, 'ArkWebSecurity',
27.                     `run JavaScript error, ErrorCode: ${(error as BusinessError).code}, Message: ${(error as BusinessError).message}`);
28.                   return;
29.                 }
30.                 if (result) {
31.                   this.webResult = result;
32.                   hilog.info(0x0000, 'ArkWebSecurity', `The test() return value is: ${result}`);
33.                 }
34.               });
35.             if (event) {
36.               hilog.info(0x0000, 'ArkWebSecurity', 'url: ', event.url);
37.             }
38.           } catch (error) {
39.             hilog.error(0x0000, 'ArkWebSecurity',
40.               `ErrorCode: ${(error as BusinessError).code}, Message: ${(error as BusinessError).message}`);
41.           }
42.         })
43.     }
44.   }
...
[RunJavaScript2.ets](https://gitee.com/harmonyos_samples/BestPracticeSnippets/blob/master/ArkWebSecurity/arkwebsecurity/src/main/ets/pages/RunJavaScript2.ets#L17-L63)
...
6. struct WebComponent {
7.   controller: webview.WebviewController = new webview.WebviewController();
8.   unknownUrl: string = ''; // unknown_url from external source
...
12.         .onClick(() => {
13.           try {
14.             // unknown_url is not verified.
15.             webview.WebCookieManager.configCookieSync(this.unknownUrl, 'a=b');
16.           } catch (error) {
17.             hilog.error(0x0000, 'ArkWebSecurity',
18.               `ErrorCode: ${(error as BusinessError).code}, Message: ${(error as BusinessError).message}`);
19.           }
20.         })
21.       Web({ src: this.unknownUrl, controller: this.controller })
22.     }
23.   }
24. }

[SetCookies.ets](https://gitee.com/harmonyos_samples/BestPracticeSnippets/blob/master/ArkWebSecurity/arkwebsecurity/src/main/ets/pages/ErrorSamples/SetCookies.ets#L17-L42)
...
6. struct WebComponent {
7.   controller: webview.WebviewController = new webview.WebviewController();
8.   unknown_url: string = ''; // unknown_url from external source
9.   urlTrustList: string =
10.     '{\'UrlPermissionList\':[{\'scheme\':\'http\', \'host\':\'trust.example.com\', \'port\':80, \'path\':\'test\'}]}'
...
14.         .onClick(() => {
15.           try {
16.             // Set up an allowlist to allow access only to trusted web pages.
...
21.           }
22.           try {
23.             // Perform another validity check on unknown_url to implement more granular control.
24.             if (this.unknown_url === 'https://www.example.com') {
25.               webview.WebCookieManager.configCookieSync(this.unknown_url, 'a=b');
26.             }
27.           } catch (error) {
28.             hilog.error(0x0000, 'ArkWebSecurity',
29.               `ErrorCode: ${(error as BusinessError).code}, Message: ${(error as BusinessError).message}`);
30.           }
31.         })
32.       Web({ src: this.unknown_url, controller: this.controller })
33.     }
34.   }
35. }

[SetCookies.ets](https://gitee.com/harmonyos_samples/BestPracticeSnippets/blob/master/ArkWebSecurity/arkwebsecurity/src/main/ets/pages/SetCookies.ets#L17-L53)
...
7.   accessToken: string = 'xxxx'; // After the user completes the login operation, the token returned by the server.
8.   untrustedURL: string = '' // Any URL to be loaded

9.   build() {
10.     Column() {
11.       Web({ src: this.untrustedURL + '?token=' + this.accessToken, controller: this.webviewController })
12.     }
13.   }
14. }

[LoadURLWithSensitiveData.ets](https://gitee.com/harmonyos_samples/BestPracticeSnippets/blob/master/ArkWebSecurity/arkwebsecurity/src/main/ets/pages/ErrorSamples/LoadURLWithSensitiveData.ets#L17-L32)
...
[PostMessage.ets](https://gitee.com/harmonyos_samples/BestPracticeSnippets/blob/master/ArkWebSecurity/arkwebsecurity/src/main/ets/pages/ErrorSamples/PostMessage.ets#L42-L44)
